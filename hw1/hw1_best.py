...
mean,std= np.load("param.npy")
net = torch.load("nn.pkl").to(device)
...
xa = torch.tensor(BN(modify_data(x_,sf),mean,std),dtype=torch.float).to(device)
ya = net(xa).view(-1)
title=[]
for i in range(500):
    title.append("id_"+str(i))
pd.DataFrame({'id':title,'value':ya.detach().cpu().numpy()}).to_csv(w_path,index=False)